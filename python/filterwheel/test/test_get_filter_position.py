...
parser = argparse.ArgumentParser(prog='test_get_filter_position.py',
                    description='Get the current position of the filter wheel (which filter is in the beam) by sending a request to the API server.')
...
urlname = "http://" + str(args.ip_address) + ":" + str(args.port_number) + "/getFilterPosition"
