...
public class DAY_CALIBRATEImplementation extends CALIBRATEImplementation implements JMSCommandImplementation
{
	 ...
	 */
	protected final static String LIST_KEY_STRING = "loci.day_calibrate.";
	/**
	 ...
	 * @see LociTCPServerConnectionThread#getMinAcknowledgeTime
	 */
	...
	// Get the amount of time to readout and save a full frame
		try
		{
			readoutOverhead = status.getPropertyInteger("loci.day_calibrate.readout_overhead");
		}
		catch (Exception e)
		{
			...
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2200);
	...
	// get fits headers
		...
		catch(Exception e )
		{
			...
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2207);
		...
		if(setFitsHeaders(dayCalibrateCommand,dayCalibrateDone) == false)
			return dayCalibrateDone;
		loci.log(Logging.VERBOSITY_INTERMEDIATE,this.getClass().getName()+
			   ":processCommand:Set Filter Wheel FITS Headers.");
		if(setFilterWheelFitsHeaders(dayCalibrateCommand,dayCalibrateDone) == false)
			return dayCalibrateDone;
	...
	// send an ack before make master processing, so the client doesn't time out.
		makeBiasAckTime = status.getPropertyInteger("loci.day_calibrate.acknowledge_time.make_bias");
	...
	// Call pipeline to create master bias.
		...
		//	return dayCalibrateDone;
	// return done
		dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_NO_ERROR);
		...
		while(done == false)
		{
			...
			// create calibration instance, and set it's type
				...
				catch(Exception e)
				{
					String errorString = new String(dayCalibrateCommand.getId()+
						...
						" at index "+index+".");
					loci.error(this.getClass().getName()+":"+errorString,e);
					dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2203);
					dayCalibrateDone.setErrorString(errorString);
					dayCalibrateDone.setSuccessful(false);
					return false;
				}
			// get common parameters
				...
				catch(Exception e)
				{
					...
					loci.error(this.getClass().getName()+":"+errorString,e);
					dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2204);
					dayCalibrateDone.setErrorString(errorString);
					dayCalibrateDone.setSuccessful(false);
					return false;
				}
			// set calibration data
				...
				catch(Exception e)
				{
					...
					loci.error(this.getClass().getName()+":"+errorString,e);
					dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2205);
					dayCalibrateDone.setErrorString(errorString);
					dayCalibrateDone.setSuccessful(false);
					return false;
				}
	...
	// get stateFilename from properties
		try
		{
			stateFilename = status.getProperty("loci.day_calibrate.state_filename");
		}
		catch (Exception e)
		{
			String errorString = new String(dayCalibrateCommand.getId()+
			...
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2201);
			dayCalibrateDone.setErrorString(errorString);
			dayCalibrateDone.setSuccessful(false);
			return false;
		}
	// initialise and load dayCalibrateState instance
		...
		catch (Exception e)
		{
			...
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2202);
			dayCalibrateDone.setErrorString(errorString);
			dayCalibrateDone.setSuccessful(false);
			return false;
		}
	...
	// update state
		...
		catch(IOException e)
		{
			...
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2206);
			dayCalibrateDone.setErrorString(errorString);
			dayCalibrateDone.setSuccessful(false);
			return false;
		}
	...
	protected boolean doConfig(DAY_CALIBRATE dayCalibrateCommand,DAY_CALIBRATE_DONE dayCalibrateDone,int bin)
	{
		try
		{
			...
			// update status config with binning, used for setting FITS headers later
			status.setConfigBinning(bin,bin);
		}
		catch(Exception e)
		{
			String errorString = new String(dayCalibrateCommand.getId()+
				":doConfig:Failed to configure CCD:");
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2208);
			dayCalibrateDone.setErrorString(errorString);
			dayCalibrateDone.setSuccessful(false);
			return false;
		}
	...
	// This is queried when saving FITS headers to get the CONFIGID value.
		...
		catch(Exception e)
		{
			...
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2209);
	 ...
	 * <li>The frame is taken. If the <b>type</b> is BIAS, <b>sendTakeBiasFrameCommand</b> is invoked 
	 *     otherwise <b>sendTakeDarkFrameCommand</b> is used.
	 * <li>testAbort is called to see if this command implementation has been aborted.
	 ...
	 * @see HardwareImplementation#testAbort
				   ...
				   int type, int exposureTime, int count)
	{
		...
		status.setExposureCount(count);
		for(int i = 0;i < count; i++)
		{
			status.setExposureNumber(i);
			if(type == DAY_CALIBRATECalibration.TYPE_BIAS)
			{
				if(setPerFrameFitsHeaders(dayCalibrateCommand,dayCalibrateDone,
							  FitsHeaderDefaults.OBSTYPE_VALUE_BIAS,0,count,i+1,
			...
			{
				if(setPerFrameFitsHeaders(dayCalibrateCommand,dayCalibrateDone,
							  FitsHeaderDefaults.OBSTYPE_VALUE_DARK,exposureTime,count,i+1,
							  status.getConfigBinningX()) == false)
					return false;
			}
		...
		// do exposure
			try
			{
				if(type == DAY_CALIBRATECalibration.TYPE_BIAS)
				{
					filename = sendTakeBiasFrameCommand((i==0));
				}
				else if (type == DAY_CALIBRATECalibration.TYPE_DARK)
				{
					filename = sendTakeDarkFrameCommand(exposureTime,(i==0));
				}
			}
			catch(Exception e)
			{
				...
				dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2210);
				dayCalibrateDone.setErrorString(errorString+e);
				dayCalibrateDone.setSuccessful(false);
				return false;
			}
		...
		int timeToComplete)
	{
		...
		catch(IOException e)
		{
			...
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2213);
			dayCalibrateDone.setErrorString(errorString+e);
			dayCalibrateDone.setSuccessful(false);
			return false;
		}
	...
	// send acknowledge to say frame is completed.
		...
		catch(IOException e)
		{
			...
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2211);
			dayCalibrateDone.setErrorString(errorString+e);
			dayCalibrateDone.setSuccessful(false);
			return false;
		}
	...
	// send acknowledge to say frame is completed.
		...
		catch(IOException e)
		{
			...
			loci.error(this.getClass().getName()+":"+errorString,e);
			dayCalibrateDone.setErrorNum(LociConstants.LOCI_ERROR_CODE_BASE+2212);
			dayCalibrateDone.setErrorString(errorString+e);
			dayCalibrateDone.setSuccessful(false);
			return false;
		}
